...
from utils.utils import Utils, CodexTokenizer
...
class InFileContextProcessor:
    ...
    def get_infile_context(self, example, simple_token_limit=1048, use_structed=False, structed_prompt_len=512):
        ...
        if 'col' not in example['metadata']:
            ...
            pre_code_lines = code_lines[:example['metadata']['context_start_lineno']] + example['prompt'].splitlines()
            # simple_in_len = self.tokenizer.tokenize(example['prompt'])
        else:
