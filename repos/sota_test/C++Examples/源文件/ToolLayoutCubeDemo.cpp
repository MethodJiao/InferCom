...
static DlgLayoutCubeDemo* m_dlg;
ToolLayoutCubeDemo::ToolLayoutCubeDemo()
{
	m_eLayoutWay = CubeLayoutWay::OnePoint;
	m_nHeight = 3000;
	m_nLength = 1000;
	m_nWidth = 200;
}
...
void ToolLayoutCubeDemo::_onPostInstall()
{
	//调用基类
	...
	if (m_dlg == nullptr)
	{
		...
		m_dlg = new DlgLayoutCubeDemo;
		m_dlg->Create(DlgLayoutCubeDemo::IDD, pView);
		m_dlg->ShowWindow(SW_SHOW);
		m_eLayoutWay = (CubeLayoutWay)m_dlg->m_nInputWay;
	}
	else
		m_dlg->ShowWindow(SW_SHOW);
	//打开捕捉
...
void ToolLayoutCubeDemo::_onRestartTool()
{
	//重启工具
	ToolLayoutCubeDemo* newTool = new ToolLayoutCubeDemo();
	newTool->installTool();
}

bool ToolLayoutCubeDemo::_onDataButton(BPBaseButtonEventCP ev)
{
	//获取鼠标屏幕点击的点
...
bool ToolLayoutCubeDemo::_onResetButton(BPBaseButtonEventCP)
{
	//点击右键退出工具
	_exitTool();
	return true;
...
void ToolLayoutCubeDemo::_onDynamicFrame(BPBaseButtonEventCP ev)
{
	if (NULL == ev)
		return;
...
bool ToolLayoutCubeDemo::_onModelMotion(BPBaseButtonEventCP ev)
{
...
bool ToolLayoutCubeDemo::_onKeyTransition(bool wentDown, ::p3d::platform::P3DVirtualKey key, bool shiftIsDown, bool ctrlIsDown)
{
	...
	case ::p3d::platform::P3DVirtualKey::Down:
		m_eLayoutWay = CubeLayoutWay::Draw;
		break;	
	}
...
void ToolLayoutCubeDemo::__addCube(PModelId modelId)
{
	...
	valueExtend.m_value.m_valueString = _T("123");
	...
	BIMBase::BPColorDef colorDef(255, 0, 0);
	...
	if (m_eLayoutWay != Multi)
		m_vctPts.clear();
}

void ToolLayoutCubeDemo::__createOnePtData(GePoint3d ptOri)
{
...
void ToolLayoutCubeDemo::__createDrawData(GePoint3d ptOri, GePoint3d ptSecond)
{
	//根据长度方向确定X轴
...
//对工具进行注册
BPTool* CreateCubeDemoTool()
{
	ToolLayoutCubeDemo* tool = new ToolLayoutCubeDemo();
	return tool;
}
AutoDoRegisterFunctionsBegin
BPToolsManager::registerTool("layoutCubeDemo", &CreateCubeDemoTool);
